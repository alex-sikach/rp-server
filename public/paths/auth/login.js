...
async function login(req, res) {
    try {
        if (req.cookies.session) {
            ...
            const session = (await pool.query('SELECT expires, open, user_id FROM sessions WHERE id = $1', [sessionId])).rows[0];
            ...
            if (open && Date.now() < expires) {
                const user = (await pool.query('SELECT username, name, lastname, avatar, theme FROM users WHERE id = $1', [id])).rows[0];
                const { password } = req.body;
                const data = { ...user, id, password, expires };
        ...
        const user = (await pool.query('SELECT id, password, avatar, name, lastname, theme FROM users WHERE username = $1', [username])).rows;
